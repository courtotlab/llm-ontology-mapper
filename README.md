...
When enabled, candidates must belong natively to one of the requested target ontologies. When disabled (the default), existing ontology-specific eligibility rules such as EFO imported-term support remain available. This is a generic flag — it does not change eligibility for ontologies that have no provenance carve-out (HPO, MONDO, LOINC, SNOMED, and so on); it only disables the EFO imported-term exception described above. It is opt-in per call, only supported in planned mode, and stable across a whole `map_data_dictionary()` batch.
...
# Lenient (default): a candidate retrieved through EFO but native to HPO/MONDO/etc. may still be eligible.
result = mapper.map_term("disease")

# Strict: only candidates whose native ontology is EFO are eligible; otherwise UNKNOWN:UNMAPPED.
...
- [**Aaryan Kandwal**](https://github.com/kandwalaaryan)
