...
# =============================================================================
# EDIT THIS SECTION FOR PUBLIC SMOKE TESTING
# =============================================================================
...
# All code prefixes that the public retrieval routes can legitimately emit,
        ...
        if allowed is not None:
            ...
            if strict_target_ontology or "EFO" not in allowed:
                assert alt_ontology in allowed, (
                    ...
                    f"TARGET_ONTOLOGY allow-list {sorted(allowed)!r} under "
                    f"strict_target_ontology={strict_target_ontology}"
                )
...
def _print_context(
    *,
    ...
    context: dict[str, Any] = {
        ...
        "target_ontology": (
            ",".join(target_ontologies) if target_ontologies else None
        ),
...
def _run_case(
    ...
    *,
    ...
    if SMOKE_DEBUG:
        ...
        assert query_plan.get("source_description") == _optional(
            SOURCE_DESCRIPTION
        ...
        assert query_plan.get("expanded_queries"), (
            "Expected non-empty expanded_queries on the actual QueryPlan "
            "debug metadata"
        )
    ...
    if override_case:
        ...
        if STRICT_TARGET_ONTOLOGY or "EFO" not in allowed:
            assert result_ontology in {*allowed, "UNKNOWN"}, (
                f"Expected {sorted(allowed)!r}/UNKNOWN in override case under "
                f"strict_target_ontology={STRICT_TARGET_ONTOLOGY}, "
                f"got {result.ontology!r}"
            )
        else:
            assert result_ontology, (
                "Lenient EFO override result must carry a non-empty "
                "native ontology"
            )

        if "HPO" in allowed:
            assert result.ontology.upper() != "LOINC", (
                "HPO override case must not return LOINC"
            )
        ...
        #
        # EFO is special in lenient mode because an EFO-scoped search may
        ...
        if STRICT_TARGET_ONTOLOGY or "EFO" not in allowed:
            assert result_ontology in {*allowed, "UNKNOWN"}, (
                ...
                f"strict_target_ontology={STRICT_TARGET_ONTOLOGY}, "
                f"got {result.ontology!r} ({result.target_code!r})"
            )
        else:
            assert result_ontology, (
                "Lenient EFO result must carry a non-empty native ontology"
            )
        ...
        # No target constraint: the pipeline picks the best-matching ontology
        # freely. Assert only that the result carries a non-empty ontology.
        assert result.ontology, (
            "result.ontology must be non-empty in unconstrained mode, "
            f"got {result.ontology!r}"
        )
    ...
    if info:
        ...
        assert (
            ...
            "Pipeline metadata did not preserve strict_target_ontology: "
...
def main() -> None:
    ...
    _run_case(
        provider,
        target_ontologies=_target_ontologies(TARGET_ONTOLOGY),
    )
